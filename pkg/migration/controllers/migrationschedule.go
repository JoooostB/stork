		...
		for _, migration := range policyMigration {
			...
			if !m.isMigrationComplete(migration.Status) {
				...
				if err != nil {
					...
					pendingMigration.Status.Status = stork_api.MigrationStatusFailed
				}

				if pendingMigration.Status.Status == stork_api.MigrationStatusInitial {
					pendingMigration.Status.Status = stork_api.MigrationStatusPending
				}

				// Check again and update the status if it is completed
				migration.Status = pendingMigration.Status.Status
				if m.isMigrationComplete(migration.Status) {
					migration.FinishTimestamp = meta.Now()
					if pendingMigration.Status.Status == stork_api.MigrationStatusSuccessful {
						m.Recorder.Event(migrationSchedule,
							v1.EventTypeNormal,
						...
						m.Recorder.Event(migrationSchedule,
							v1.EventTypeWarning,
							string(stork_api.MigrationStatusFailed),
							fmt.Sprintf("Scheduled migration (%v) failed", migration.Name))
					}
				}
				updated = true
			}
...
func (m *MigrationScheduleController) shouldStartMigration(migrationSchedule *stork_api.MigrationSchedule) (stork_api.SchedulePolicyType, bool, error) {
...
func (m *MigrationScheduleController) formatMigrationName(migrationSchedule *stork_api.MigrationSchedule, policyType stork_api.SchedulePolicyType) string {
	return strings.Join([]string{migrationSchedule.Name, strings.ToLower(string(policyType)), time.Now().Format(nameTimeSuffixFormat)}, "-")
}

func (m *MigrationScheduleController) startMigration(migrationSchedule *stork_api.MigrationSchedule, policyType stork_api.SchedulePolicyType) error {
